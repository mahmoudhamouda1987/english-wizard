    ...
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "48px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16, marginTop: 24 }}>
        <button onClick={() => selectVariant("ACADEMIC")} className="button" style={{ padding: 0, textAlign: "left", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ background: "var(--surface-card)", padding: "28px 26px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
              <span className="pill" style={{ fontSize: 13, fontWeight: 700 }}>Academic</span>
              <div>
  ...
  if (step === "band") return (
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "48px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(88px, 1fr))", gap: 10, marginTop: 24, maxWidth: 600, margin: "24px auto 0" }}>
        {BANDS.map((b) => {
          const tier = b >= 7 ? "Advanced" : b >= 5.5 ? "Intermediate" : "Foundation";
          const bg = "var(--bg-muted)";
          return <button key={b} onClick={() => { setLoading(true); setError(""); setBand(b); }} className="button secondary" aria-label={`Band ${b} — ${tier}`} title={tier} style={{ fontSize: 22, padding: "16px 0", borderRadius: 12, background: bg, fontWeight: 700 }}>{b}</button>;
        })}
      </div>
      <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 20, fontSize: 12.5, opacity: 0.6 }}>
        <span className="pill">Foundation · 4.0–5.0</span><span className="pill">Intermediate · 5.5–6.5</span><span className="pill">Advanced · 7.0–9.0</span>
      </div>
    </main>
  );
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
        {SKILLS.map((skill) => (
          <div key={skill} className="panel" style={{ padding: 0, overflow: "hidden", borderRadius: 14 }}>
            <div style={{ background: "var(--bg-muted)", padding: "16px 20px", borderBottom: "1px solid var(--border-subtle)" }}>
...
function ModuleRunner({ variant, band, skill, stage, onBack, onComplete }: { variant: IeltsVariant; band: number; skill: string; stage: string; onBack: () => void; onComplete: () => void }) {
    ...
    <main style={{ maxWidth: 820, margin: "0 auto", padding: "48px 24px" }}>
      ...
      <div style={{ display: "grid", gap: 14 }}>
        <div className="panel" style={{ padding: 22, textAlign: "center" }}>
          <div style={{ fontSize: 42, fontWeight: 700, color: "var(--accent-primary)" }}>{result.band}</div>
  ...
  return (
    <main style={{ maxWidth: 820, margin: "0 auto", padding: "48px 24px" }}>
      ...
      {skill === "writing" && writingTask && <div className="panel" style={{ padding: 22, marginBottom: 18, borderLeft: "3px solid var(--accent-primary)", borderRadius: 14 }}><h3 style={{ margin: "0 0 10px" }}> {writingTask.title}</h3><p style={{ margin: "0 0 12px", lineHeight: 1.6 }}>{writingTask.prompt}</p>{writingTask.data && <pre style={{ fontSize: 13, whiteSpace: "pre-wrap", background: "var(--bg-muted)", padding: 12, borderRadius: 8 }}>{writingTask.data.join("\n")}</pre>}<textarea value={writingText} onChange={(e) => setWritingText(e.target.value)} rows={10} placeholder="Write your response here…" style={{ width: "100%", marginTop: 12, fontSize: 14, lineHeight: 1.6, borderRadius: 8, border: "1px solid var(--border-default)", padding: 12 }} /><p className="subtle">Word count: {writingText.trim().split(/\s+/).filter(Boolean).length} / {writingTask.minWords}</p></div>}
      {skill === "speaking" && speakingCard && <div className="panel" style={{ padding: 22, marginBottom: 18, borderLeft: "3px solid var(--accent-primary)", borderRadius: 14 }}><h3 style={{ margin: "0 0 10px" }}> Cue card</h3><p style={{ margin: "0 0 8px", fontWeight: 600 }}>{speakingCard.topic}</p><ul style={{ margin: "0 0 16px", paddingLeft: 20, lineHeight: 1.7 }}>{speakingCard.prompts.map((p, i) => <li key={i}>{p}</li>)}</ul><button className="button secondary" onClick={() => { speakText(`Describe ${speakingCard.topic.toLowerCase()}.`, { lang: "en-GB", gender: "female" }); }}>Play prompt aloud</button></div>}
      {items.length > 0 && <div className="panel" style={{ padding: 22, marginBottom: 18, borderRadius: 14 }}><h3 style={{ margin: "0 0 14px" }}> Questions ({items.length})</h3><div style={{ display: "grid", gap: 14 }}>{items.map((item, idx) => <div key={item.id} style={{ padding: "14px 16px", background: "var(--bg-muted)", borderRadius: 10, border: "1px solid var(--border-default)" }}>
