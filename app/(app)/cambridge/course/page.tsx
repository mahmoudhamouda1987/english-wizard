...
const QUALIFICATIONS = [
  { id: "A2_KEY", name: "A2 Key", cefr: "A2", tier: "foundation", desc: "Elementary level — basic everyday English." },
  { id: "B1_PRELIMINARY", name: "B1 Preliminary", cefr: "B1", tier: "foundation", desc: "Intermediate — work, study and travel contexts." },
  { id: "B2_FIRST", name: "B2 First", cefr: "B2", tier: "higher", desc: "Upper-intermediate — confident, effective communication." },
  { id: "C1_ADVANCED", name: "C1 Advanced", cefr: "C1", tier: "higher", desc: "Advanced — complex, demanding professional situations." },
  { id: "C2_PROFICIENCY", name: "C2 Proficiency", cefr: "C2", tier: "mastery", desc: "Mastery — near-native fluency and precision." },
] as const;

const KINDS = [
  { id: "readiness-assessment", label: "Exam readiness check", desc: "Full diagnostic across all skills." },
  { id: "vocabulary-benchmark", label: "Vocabulary benchmark", desc: "Test your word knowledge at this level." },
  { id: "grammar-benchmark", label: "Grammar & Use of English", desc: "Sentence structure and accuracy." },
  { id: "reading-benchmark", label: "Reading benchmark", desc: "Comprehension and inference skills." },
  { id: "listening-benchmark", label: "Listening benchmark", desc: "Audio understanding and note-taking." },
  { id: "writing-task", label: "Writing task", desc: "Structured writing with timed conditions." },
  { id: "speaking-card", label: "Speaking card", desc: "Part 2 long turn with cue card." },
] as const;
    ...
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "48px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
        {QUALIFICATIONS.map((q) => (
          <button key={q.id} onClick={() => setQualId(q.id)} className="button secondary" style={{ padding: 0, textAlign: "left", borderRadius: 14, overflow: "hidden", border: "1px solid var(--border-default)" }}>
            <div style={{ background: "var(--surface-card)", padding: "20px 22px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                <span className="pill" style={{ fontSize: 12, fontWeight: 700 }}>{q.cefr}</span>
                <div>
  ...
  if (!kind) return (
    <main style={{ maxWidth: 960, margin: "0 auto", padding: "48px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <span className="pill" style={{ fontSize: 14, fontWeight: 700 }}>{qual?.cefr}</span>
        ...
        {KINDS.map((k) => (
          <button key={k.id} onClick={() => { setLoading(true); setError(""); setKind(k.id); }} className="button secondary" style={{ padding: 0, textAlign: "left", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start", padding: "16px 18px" }}>
              <span className="pill" style={{ fontSize: 12, fontWeight: 600, flexShrink: 0 }}>{k.label.split(" ")[0]}</span>
              <div>
  ...
  if (result) return (
    <main style={{ maxWidth: 820, margin: "0 auto", padding: "48px 24px" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <span className="pill" style={{ fontSize: 16, fontWeight: 700 }}>{qual?.cefr}</span>
      ...
      <div style={{ display: "grid", gap: 14 }}>
        <div className="panel" style={{ padding: 22, textAlign: "center" }}>
          <div style={{ fontSize: 42, fontWeight: 700, color: "var(--accent-primary)" }}>{result.scaleEstimate}</div>
  ...
  return (
    <main style={{ maxWidth: 820, margin: "0 auto", padding: "48px 24px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <p className="eyebrow">Cambridge · {qual?.name}</p>
      ...
      {assessment && <>
        ...
        {assessment.writingPrompt && (
          <div className="panel" style={{ padding: 22, marginBottom: 16, borderLeft: "3px solid var(--accent-primary)" }}>
            <h3 style={{ margin: "0 0 10px" }}> Writing task</h3>
            <p style={{ margin: "0 0 12px", lineHeight: 1.6 }}>{assessment.writingPrompt}</p>
            <textarea value={writingText} onChange={(e) => setWritingText(e.target.value)} rows={10} placeholder="Write your response here…" style={{ width: "100%", fontSize: 14, lineHeight: 1.6, borderRadius: 8, border: "1px solid var(--border-default)", padding: 12 }} />
          ...
          <div className="panel" style={{ padding: 22, marginBottom: 16 }}>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              {assessment.objectiveItems.map((item, idx) => (
                <div key={item.id} style={{ padding: "14px 16px", background: "var(--bg-muted)", borderRadius: 10, border: "1px solid var(--border-default)" }}>
                  ...
                  {item.kind === "gap" && <input value={answers[item.id] ?? ""} onChange={(e) => setAnswers((a) => ({ ...a, [item.id]: e.target.value }))} placeholder="Type your answer…" style={{ padding: "10px 14px", fontSize: 14, borderRadius: 8, border: "1px solid var(--border-default)", width: "100%" }} />}
                </div>
              ))}
            </div>
          </div>
      ...
      {error && <p role="alert" style={{ color: "var(--danger)", padding: "10px 16px", background: "var(--bg-muted)", borderRadius: 8 }}>{error}</p>}
