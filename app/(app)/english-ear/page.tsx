...
export default function EnglishEarPage(){
 ...
 const [busy,setBusy]=useState(false);const [playing,setPlaying]=useState(false);
 useEffect(()=>{fetch("/api/profile").then(r=>r.json()).then(p=>{if(p.profile?.targetLevel)setLevel(p.profile.targetLevel);}).catch(()=>{});},[]);
 function speak(text:string){setPlaying(true);speakText(speechFriendly(text),{lang:"en-GB",rate:0.9,onEnd:()=>setPlaying(false)});}
